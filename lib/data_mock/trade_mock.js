...
var createTrade = function(params, customer) {
    ...
    trade.orders = orders;
    trade.tid = 'T' + new Date().getTime();
    trade.title = '测试交易';
    trade.status = 'WAIT_BUYER_PAY';
    trade.payment = payment;
    trade.modified = trade.create_time;
...
var createOrder = function(param) {
    ...
    trade.create_time = param.create_time;
    order.pay_time = param.pay_time;
    order.consign_time = param.consign_time;
    order.end_time = param.end_time;
